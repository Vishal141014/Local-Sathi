...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const pendingSubscriptions = await Subscription.countDocuments({ status: 'pending_verification' });
    const pendingVerifications = await Subscription.find({ status: 'pending_verification' })
...
router.get('/messages', async (req, res) => {
  try {
    /* Temporarily disabled until Message model is implemented
...
router.get('/subscriptions/analytics', async (req, res) => {
  try {
    ...
    const activeSubscriptions = await Subscription.countDocuments({ status: 'active' });
    const pendingVerifications = await Subscription.countDocuments({ status: 'pending_verification' });
